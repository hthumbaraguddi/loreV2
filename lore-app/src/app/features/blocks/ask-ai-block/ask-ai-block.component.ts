...
import {
  ...
  PROVIDER_REGISTRY,
  ProviderDefinition,
} from '../../../core/config/provider-registry';
    ...
    <div class="blk-ai-response">
      ...
      <div class="blk-ai-hdr">
        ...
        <div class="blk-ai-provider-wrap">
          <button
            ...
            [disabled]="loading() || hasFixedProvider()"
            [title]="
              hasFixedProvider()
                ? 'Provider fixed by wrapper'
                : 'Switch AI provider'
            "
          ...
          >
            ...
            <span class="material-symbols-outlined picker-chevron"
              >expand_more</span
            >
          </button>
            ...
            >
              @for (p of allProviders; track p.id) {
                ...
                >
                  <span
                    class="opt-dot"
                    [style.background]="providerColorFor(p.id)"
                  ></span>
      ...
      @if (!hasApiKey()) {
        <div class="blk-ai-no-key" role="alert">
          ...
          <a
            class="blk-ai-settings-link"
            routerLink="/settings"
            fragment="ai-providers"
          >
            Add key in Settings →
          </a>
        </div>
      ...
      <textarea
        ...
        [placeholder]="
          hasApiKey()
            ? 'Ask ' + providerName() + ' about this note… (⌘↵ to submit)'
            : 'Configure an API key to use this block'
        "
      ></textarea>
      ...
      @if (currentResponse()) {
        <div class="blk-ai-resp-area">
          ...
          <div class="blk-ai-resp-meta">
            <span class="resp-timestamp">{{
              currentResponse()!.timestamp | date: 'short'
            }}</span>
            <span class="resp-model">{{ currentResponse()!.model }}</span>
            @if (currentResponse()!.tokensUsed) {
              <span class="resp-tokens"
                >{{ currentResponse()!.tokensUsed }} tokens</span
              >
            }
          </div>
        </div>
  ...
  styles: [
    `
      ...
      .blk-ai-provider-btn {
        ...
        font-size: 12px;
        font-weight: 600;
        color: var(--lore-primitive-purple-600);
      ...
      .blk-ai-status {
        margin-left: auto;
        font-size: 10.5px;
        color: var(--lore-primitive-green-700);
        background: var(--lore-primitive-green-50);
        padding: 2px 8px;
        border-radius: 3px;
        font-family: 'JetBrains Mono', monospace;
        display: flex;
      ...
      .status-dot {
        ...
        .streaming & {
          background: var(--lore-primitive-purple-600);
        }
      }
      /* No API key warning */
      .blk-ai-no-key {
      ...
      .blk-ai-prompt {
        ...
        &:focus {
          border-color: var(--lore-primitive-purple-400);
        }
        &:disabled {
          opacity: 0.5;
          cursor: not-allowed;
      ...
      .blk-ai-run {
        ...
        &:hover:not(:disabled) {
          background: var(--lore-primitive-purple-700);
        }
        &:disabled {
          opacity: 0.5;
          cursor: not-allowed;
      ...
      .blk-ai-btn {
        ...
        &:hover:not(:disabled) {
          color: var(--lore-primitive-purple-600);
          border-color: var(--lore-primitive-purple-300);
        }
        &:disabled {
          opacity: 0.4;
          cursor: not-allowed;
...
export class AskAiBlockComponent implements OnInit {
  ...
  // ── Computed ───────────────────────────────────────────────────────────────
  hasFixedProvider = computed(() => !!this.provider());

  // ── Computed ──────────────────────────────────────────────────────────────
  currentResponse = computed(
    () => this.responses()[this.currentIndex()] ?? null,
  );
  hasPrevious = computed(() => this.currentIndex() > 0);
  hasNext = computed(() => this.currentIndex() < this.responses().length - 1);
  providerName = computed(() => {
    const providerId = this.provider() || this.activeProvider();
    return PROVIDER_MAP.get(providerId)?.displayName ?? providerId;
  });
  providerInitial = computed(() => this.providerName().charAt(0).toUpperCase());
  hasApiKey = computed(() =>
    this.keyManager.hasKey(this.provider() || this.activeProvider()),
  );
  ...
  ngOnInit(): void {
    ...
    if (this.provider()) {
      this.activeProvider.set(this.provider() as string);
  ...
  toggleProviderPicker(): void {
    if (this.hasFixedProvider()) return;
  ...
  providerColorFor(id: string): string {
    switch (id) {
      case 'anthropic':
        return 'var(--lore-primitive-purple-600)';
      case 'openai':
        return '#10A37F';
      case 'google':
        return '#4285F4';
