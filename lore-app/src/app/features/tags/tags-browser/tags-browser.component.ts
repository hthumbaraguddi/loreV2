      ...
      <div class="browser-header">
        <div class="header-content">
          ...
          <p class="page-subtitle">
            Browse and manage all tags across your notes
          </p>
        </div>
              ...
              @for (tag of filteredTags(); track tag.name) {
                <button class="tag-card" (click)="selectTag(tag.name)">
                  <div class="tag-card-header">
                    <div class="tag-left">
                      <div
                        class="tag-color-dot"
                        [style.background]="
                          tag.color || 'var(--lore-color-accent)'
                        "
                      ></div>
                      <div class="tag-name">{{ tag.name }}</div>
                    </div>
                    <div class="tag-count" aria-label="Notes count">
                      {{ tag.count }}
                    </div>
                  </div>
                  ...
                  <div class="tag-card-footer">
                    <button
                      class="tag-action"
                      (click)="editTag($event, tag.name)"
                    >
                      <span class="material-symbols-outlined">edit</span>
                    </button>
                    <button
                      class="tag-action delete"
                      (click)="deleteTag($event, tag.name)"
                    >
              ...
              @for (tag of filteredTags(); track tag.name) {
                <div class="tag-row">
                  <button
                    ...
                    [attr.aria-label]="'View tag ' + tag.name"
                  >
                    ...
                    <div class="tag-row-notes">
                      ...
                      @if (tag.notes.length > 2) {
                        <span class="note-badge more"
                          >+{{ tag.notes.length - 2 }}</span
                        >
                      }
                    </div>
                  </button>
                  <div class="tag-row-count" aria-hidden="true">
                    {{ tag.count }}
                  </div>
