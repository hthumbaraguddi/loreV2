      ...
      <div class="tbl-header">
        ...
        @if (!readOnly()) {
          <div class="tbl-actions">
            <button class="tbl-action" type="button" (click)="addRow()">+ Row</button>
            <button class="tbl-action" type="button" (click)="addColumn()">+ Col</button>
          </div>
        }
      </div>
              ...
              <tr>
                @for (cell of row; track cell; let colIndex = $index) {
                  <td
                    ...
                    [attr.data-placeholder]="'Cell ' + (rowIndex + 1) + ', ' + (colIndex + 1)"
                  ></td>
                }
              </tr>
            }
...
export class TableBlockComponent implements OnInit {
  ...
  ngOnInit(): void {
    const metadata = (this.block().metadata ?? {}) as { headers?: string[]; rows?: string[][] };
    this.headers.set(Array.isArray(metadata.headers) && metadata.headers.length ? metadata.headers : ['Column 1', 'Column 2']);
    this.rows.set(Array.isArray(metadata.rows) && metadata.rows.length ? metadata.rows : [['', '']]);
  }

  onHeaderInput(event: Event, index: number): void {
    const text = (event.target as HTMLElement).innerText;
    this.headers.set(this.headers().map((header, idx) => (idx === index ? text : header)));
    this.emitChange();
  }

  onCellInput(event: Event, rowIndex: number, colIndex: number): void {
    const text = (event.target as HTMLElement).innerText;
    this.rows.set(this.rows().map((row, rIdx) =>
      rIdx !== rowIndex
        ? row
  ...
  addColumn(): void {
    this.headers.set([...this.headers(), `Column ${this.headers().length + 1}`]);
